...
## Confidence Triage — ECE Monitoring Without Threshold Drift Control
[R-26.4.22, ai-agent-rollout-playbook-vet] Standard playbooks correctly specify Brier score and Expected Calibration Error (ECE) as diagnostic signals for confidence-based routing decisions (e.g., routing low-confidence outputs to human review). Gap: neither standard playbook specifies (a) initial ECE threshold that triggers routing changes, (b) who monitors ECE drift post-deployment, (c) recomputation cadence, or (d) intervention logic when ECE degrades. The LLM ECE systematic range is 0.12-0.40; silent vendor model updates shift confidence distributions without notification — a model update can increase or decrease confidence scores across the same query distribution without any explicit version change signal. Without threshold drift monitoring, confidence triage degrades silently: outputs that should route to human review auto-execute as model confidence distributions shift post-update. ECE is named as a metric in standard playbooks; it needs to function as a monitored control with explicit intervention logic. Required additions: (a) baseline ECE measurement at shadow-mode exit with threshold for acceptable drift, (b) automated recomputation trigger on every vendor model update (not just quarterly), (c) defined intervention when ECE exceeds drift threshold (route to human review until recalibrated). Severity MEDIUM — becomes HIGH when confidence triage is load-bearing for human gate decisions at Tier 2+ write-capable deployments. Source: CDS-2 F[CDS-A5] (independent research T2).
...
- R-26.4.22, ai-agent-rollout-playbook-vet, retroactive 06b compilation 26.5.5 — additional finding from F[CDS-A5] (ECE confidence-triage threshold drift control absent)
