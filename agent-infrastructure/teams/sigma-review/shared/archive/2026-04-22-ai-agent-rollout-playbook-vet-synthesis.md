...
Neither playbook addresses multi-agent trust models. Both treat agents as isolated entities under single-user OBO tokens. In multi-agent topologies — now the default deployment pattern in LangGraph (which LangSmith Q1 2026 telemetry shows has more than 60% of production deployments using multi-agent subgraph patterns with default credential inheritance) — orchestrator compromise via indirect prompt injection allows subagents to execute actions with the orchestrator's full scope. MITRE ATLAS added technique AML.T0051.002 "LLM Prompt Injection via Intermediate Agent" in its March 2026 update, classifying chain-of-agent injection as a production technique category.
...
- MITRE ATLAS AML.T0051.002 "LLM Prompt Injection via Intermediate Agent" was added in March 2026 as a production technique category — the multi-agent security gap has moved from theoretical to classified-production-relevant.
