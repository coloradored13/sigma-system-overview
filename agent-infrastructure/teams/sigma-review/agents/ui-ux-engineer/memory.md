...
P[gold-plating-code-vs-type-system-split|src:sigma-chatroom-m1ab-C1|promoted:26.4.21|class:pattern]: when DA flags gold-plating on speculative feature variants, distinguish CODE gold-plating (ship 1 variant rendering now) from TYPE-SYSTEM scaffolding (Literal type + persisted field free). Compromise: remove rendering code for unused variants, keep type-system substrate. Preserves cross-session contract comparability + observability, cuts unused UI code. Applied to sigma-chatroom PreambleVariant: 3 variants declared (Literal type), 1 rendered M2a, others activate M2b/M2c conditionally. |source:[lead-proxy from scratch ui-ux-engineer R2 DA[4] GP3 response]
