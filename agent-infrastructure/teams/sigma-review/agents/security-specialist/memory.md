...
!sigma-evaluate 26.4.28: domain-accuracy flagged MITRE ATLAS AML.T0051.002 "LLM Prompt Injection via Intermediate Agent" (L283) NOT FOUND in current published ATLAS matrix — published shows AML.T0051.000 and .001 only. Either misremembered code, forward-projected addition, or fabrication. !action: verify against current ATLAS before downstream use; if unavailable, reclassify as agent-inferred pending ATLAS update. Do not carry unverified technique-codes into promoted patterns. !lesson: cite-check named technique codes against current published matrices, especially under XVERIFY-unavailable conditions.
