...
P[3-tier-merge-gate-test-strategy|src:sigma-chatroom-m1ab-C1|promoted:26.4.21|class:pattern]: merge-gate round-trip tests (from_sdk(to_sdk(msg))==msg) pass with SYMMETRIC field-loss bugs — both directions lose the same field, equality still holds. Mitigation: 3-tier structure. Tier 1: hand-computed golden SDK fixtures OUTBOUND (canonical→SDK-wire). Tier 2: hand-computed golden SDK fixtures INBOUND (SDK-wire→canonical). Tier 3: round-trip as derivative smoke test. Tiers 1+2 catch symmetric-loss via independent source of truth; Tier 3 catches non-symmetric drift cheaply. Applied to sigma-chatroom message_mapping.py Anthropic content-blocks ↔ OpenAI siblings adapters. |source:[lead-proxy from scratch impl-eng R2 DA[6] response + cqa R2 G-F1/G-F2/G-F3]
P[provider-statelessness-SDK-context-manager|src:sigma-chatroom-m1ab-C1|promoted:26.4.21|class:pattern]: for multi-model LLM client libraries with tool-exec loops, Provider statelessness invariant is maintained via SDK context-manager semantics — anthropic.MessageStreamManager + openai.AsyncStream are WITHIN-CALL scope; fresh connection + accumulator per stream() call. No session state across tool-exec boundary. IC invariants: INV4 (provider stateless per call), INV5 (reconstruction-state lives in TurnEngine not Provider). Prevents subtle bugs where Provider accumulators leak across tool-exec iterations. |source:[lead-proxy from scratch impl-eng R2 DA[8] response]

## patterns (promoted from r19-remediation C1 — 26.4.23)

P[chain-evaluator-stop-hook-non-looping|src:r19-remediation-c1|promoted:26.4.23|class:calibration]
Stop hook in chain-evaluator.py fires once and exits — non-looping by design. Any fix requiring a wait/poll loop inside the Stop hook violates this invariant (adds latency to all session ends, including non-sigma sessions). For A12 archive timing: use synchronous mtime delta check (compare archive dir mtime to hook invocation time) with 24h grace window. Signal-driven re-run with timeout is the wrong pattern for a Stop hook. DA-confirmed: ADR[3] revised from signal-driven to 24h grace in r19-remediation C1. |source:[code-read chain-evaluator.py:625-640]

P[check-a3-two-layer-authority|src:r19-remediation-c1|promoted:26.4.23|class:calibration]
check_a3 in chain-evaluator.py:162-183 uses a two-layer depth model after r19-remediation: gc.check_dialectical_bootstrapping = PRESENCE check (V6, has DB[] at all); chain-evaluator check_a3 = DEPTH check (are 3-of-5 step-markers present). These are SEQUENTIAL not conflicting. Prior state (uncoordinated, gc passes while chain-evaluator appends issues) was the bug. In C2 implementation: expand marker set in check_a3 only; do NOT touch gc.check_dialectical_bootstrapping; do NOT add a third layer. Marker expansion: use alternation patterns (r"assume.?wrong|assume wrong", r"re.?estimate|re-estimate") not exact strings. Also fix section boundary from (?=DB\[|\Z|###) to (?=DB\[|\Z|#{3,}) to catch #### subheadings. |source:[code-read chain-evaluator.py:162-183, IC[4]]

P[shlex-split-sed-detection|src:r19-remediation-c1|promoted:26.4.23|class:pattern]
For phase-gate.py sed -i detection: use shlex.split() argv tokenization, NOT raw string regex. Raw `r'sed\s+-i'` blocks `sed -i ''` (BSD-safe) and `sed -i.bak` — false positives on correct usage. shlex.split() produces deterministic argv lists: `sed -i "s/x/y/" file` → ['sed', '-i', '"s/x/y/"', 'file']; `sed -i '' "s/x/y/"` → ['sed', '-i', '', '"s/x/y/"']. Detection: 'sed' in args AND '-i' in args AND next arg after '-i' is NOT a backup extension (starts with '.' or is empty string ''). MUST wrap shlex.split() in try/except ValueError (malformed quoting → deny-safe default). Known gaps: `env sed -i`, `xargs sed -i` — accepted as documented residual risk per SS ADR[1] post-XVERIFY. Also scope to workspace paths only (/.claude/teams/|workspace.md). |source:[code-read phase-gate.py:252-274, SS-ADR[1]-post-XVERIFY]

P[sigma-verify-auto-ready-startup|src:r19-remediation-c1|promoted:26.4.23|class:pattern]
sigma-verify sub-tools (verify_finding, cross_verify, challenge) are state-gated in machine.py with from_states=["ready"]. Fix: call handle_init() inside build_machine() at startup when any provider API key present — auto-transitions registry to "ready" state before first MCP connection. handle_init() is key-check-only, no external HTTP, no side effects (confirmed via handlers.py code-read). All MCP tool-list queries after server start then return full action set. Spawn-prompt init instruction (SQ[SS-3]) ships as belt-and-suspenders, not primary enforcement. Implementation: ~5 LOC in build_machine() in ~/Projects/sigma-verify/src/sigma_verify/machine.py. |source:[code-read machine.py:17-128, handlers.py]

## calibration (r19-remediation C1 — 26.4.23)

CAL[test-suite-gate-checks-baseline|date:26.4.23]: actual baseline 143 passing (TA live run) / CQA measured 92 collected 81 passing — divergence likely fixture/environment difference. C2 must run pytest at session start to establish authoritative floor before any changes. 11 pre-existing failures root cause: MINIMAL_WORKSPACE fixture uses non-roster agents (agent-alpha/agent-beta); roster.md added later broke fixture assumptions. Fix fixture before running regression tests. Zero coverage on A16/A17/A18 (peer-verification ring) — all new in this build.

CAL[new-gate-test-budget|date:26.4.23]: SQ[13a-g] total 11-15h. Per-gate estimates: A12-key-fix=2 tests; A16/A17/A18-peer-verify=8-12 tests; A3-depth=4-6 tests; A12-timing=2-3 tests; A20-precision=6-8 tests; governance-artifact=3-4 tests; severity-provenance=2-3 tests; BLOCK3-sed=8 tests (expanded matrix per CQA); fixture-fix=1-3 tests. Highest complexity+flakiness: A20 precision gate (calibration against known R19 cases required).
