...
P[onboarding-budget-weakest-link: when a protocol or spec must work across heterogeneous systems (multiple model families, multiple runtime environments), the onboarding prompt/spec complexity budget is bounded by the least capable system in the target set, not the most capable. Designing to frontier capability and assuming smaller systems will cope is an anti-pattern — it systematically fails the weakest-link systems that most need reliable onboarding. Applicable whenever cross-system compatibility is a constraint (not single-system deployments). |src:cross-model-communication-protocol-review|promoted:26.4.9|class:calibration]
P[error-taxonomy-structural-semantic-pragmatic: generic error messages ("malformed message", "invalid input") are unusable for correction in any system where the sender must fix and retry. Three categories with distinct recovery paths: structural (missing/wrong-type field → retry with repair), semantic (fields valid but values contradict → flag+amend, not blind retry), pragmatic (parseable but insufficient to act on → request clarification). Generic errors send senders into blind retry loops that reproduce the same error. This taxonomy applies beyond protocol design — useful for any API, CLI, or agent interface where the caller needs to diagnose and fix the problem. |src:cross-model-communication-protocol-review|promoted:26.4.9|class:pattern]
P[snake-case-cross-system: snake_case is the most consistent naming convention across heterogeneous systems where field names must be produced and parsed by different parsers/tokenizers/runtimes. camelCase splits inconsistently across BPE tokenizer families and is ambiguous at system boundaries (HTTP headers, SQL, Python all treat casing differently). ALL_CAPS carries semantic baggage. Constraints: avoid programming language reserved words (type, class, from, id, for, return); avoid pure abbreviations (message_id not mid); use numeric indexing for positional elements. This is load-bearing for cross-system reliability, not cosmetic. |src:cross-model-communication-protocol-review|promoted:26.4.9|class:calibration]
P[spec-decoder-separation|spec and decoder guide MUST be separate artifacts|combining degrades both|spec=normative terse model-facing|decoder=annotated rendering template for human auditors|src:cross-model-protocol-review|promoted:26.4.9|class:principle]
P[decoder-guide-is-rendering-spec|decoder guide answers "what was the agent communicating?" not "what's the schema?"|each field annotated with contextual meaning+example values+edge cases|pure schema dump insufficient for audit reconstruction|src:cross-model-protocol-review|promoted:26.4.9|class:principle]
