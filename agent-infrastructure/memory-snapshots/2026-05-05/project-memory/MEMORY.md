...
- [AI PD Tracker](project_ai-pd-tracker.md) — ~/Projects/ai-pd-tracker/ FastAPI+SQLite curriculum tracker, 10 steps shipped 26.4.20 + Field Guide work area + verified source ledger (23/23) + curriculum schema (track/gate/notebook_id) + 6 NotebookLM manifests + DEFERRED.md 26.4.29-30, private repo coloradored13/ai-pd-tracker, coach validated (shares sigma-verify API key)
...
- [Hook enforcement](project_hook-enforcement.md) — chain-evaluator.py (Stop, non-looping) + phase-gate.py (2 BLOCKs + 1 WARN). LIVE 26.4.24: 154 collected / 143 pass / 11 pre-existing fail (all SQ[0] fixture roster-mismatch; prior "lost in refactor" claim falsified — files moved not deleted)
