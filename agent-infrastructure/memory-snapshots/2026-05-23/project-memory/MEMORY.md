...
- [Cutebot MCP](project_cutebot-mcp.md) — ~/Projects/cutebot-mcp/ MCP server. v0.2.2 LIVE 26.5.23: full 22-action systems check. Both motors confirmed working (prior "left motor dead" diagnosis was wrong — hair on left axle was the friction culprit). Watchdog bug fixed (long drives now run full duration). Light/sound NOT broken — env-occluded by Cutebot mount, work fine bare-µbit (proven via new sense_debug.py diagnostic). Headlight wire protocol corrected (string side label, not numeric). Compass calibrated. ⚠️ Recurring radio wedge (~30-60s of activity → fix is Cutebot power-cycle) — root cause unknown, investigate next. 6 commits, working tree clean.
