...
500ms watchdog on the robot side guards open-ended drives (D,L,R,0). For explicit-duration drives (D,L,R,ms with ms>0) the firmware sets `auto_stop_at` as the safety deadline and the watchdog defers to it — v0.2.2 patch (26.5.23): `elif auto_stop_at == 0 and now - last_cmd_ms > WATCHDOG_MS:`. Before this patch, any drive >500ms was silently capped at 500ms by the watchdog regardless of requested duration.
...
**Motor status RE-VERIFIED 2026-05-22 (prior "left motor dead" claim was wrong):**
Both motors functional. Isolation test sequence (D,50,0 / D,0,50 / D,50,50 / D,-50,-50):
- ✅ Left motor only fwd → CW pivot as expected
- ✅ Right motor only fwd → CCW pivot as expected
- ✅ Both fwd → translates forward, but veers (left wheel acts as pivot — right side outpaces left)
- ✅ Both reverse → translates backward
- ✅ Stop responsive
**Remaining issue is the v0.1 power/friction asymmetry, not a dead motor.** Right side produces more torque OR left wheel has higher rolling resistance — observable as CCW veer under D,50,50. Discriminator test: lift bot and run D,50,50 — equal rates in air = friction; unequal = electrical. **Lesson:** prior "dead motor" diagnosis was based on a single session under degrading battery; should have re-tested across power cycles before storing as fact.
...
## Session 2026-05-23 — full systems check

Re-flashed bridge + robot µbits from current source, then exercised the full 22-action surface.

**Motors** (both work, prior "left motor dead" was wrong):
- Isolation test (D,50,0 / D,0,50 / D,50,50 / D,-50,-50): all four respond cleanly.
- Forward translation veered initially (left wheel lagged) — root cause was a hair wrapped around the left axle. After removing, lifted-bot rates look symmetrical to the eye.
- 10s drives now run full duration (after watchdog fix).

**Watchdog fix shipped** — v0.2.2 patch to `robot_microbit.py:353`. Before: every drive capped at 500ms. After: explicit-duration drives respect `auto_stop_at` deadline; open-ended drives (ms=0) still get 500ms safety net.

**Actuators**:
- Headlights work — but wire protocol uses **string side label "L"/"R"/"B"**, not numeric LED position (4/8). Prior memory had this wrong. Numeric values silently no-op in the H handler (return OK without firing set_headlight).
- Beep: two pitches audible. Capped at 200ms by firmware.
- Servo: untested (no physical servo connected).

**Distance & vision**:
- Ultrasonic: reads, but accuracy off when obstacle is shorter than ~5cm (beam goes over). 30cm obstacle read as 70-108cm.
- Line sensors: working correctly. Semantics: **1 = sees black/on-track, 0 = sees white**. Three-state test confirmed: lifted → 1,1; on white → 0,0; over line → 1,1. The wooden-table 1,1 reading from v0.1 was the documented IR false-positive on dark surfaces (memory was right about this — sensors work as designed).
- Polar scan: rotates bot through each angle, returns plausible polar distances.

**Light + sound — NOT bugs.** Both sensors work at the firmware level (proven via standalone diagnostic in [[sense-debug]]):
- `display.read_light_level()` returns valid values under direct flashlight (~200/255 — top bar 8/10 LEDs lit on diagnostic).
- `microphone.sound_level()` registers on a growl (bottom bar pulses).
- `microphone.was_event(SOUND_EVENT_LOUD)` event API present and works.
- Reason they read ~0 via MCP: **µbit's LED matrix and mic face away from typical sound/light sources** when mounted on the Cutebot (matrix faces forward, mic faces backward into the body). The sensors are environmentally occluded, not broken. **Potential v0.3 improvement**: add `read_sound_peak` action that samples sound over a 500ms window and returns max — more useful than instantaneous `sound_level()` for clap-style detection.

**IMU**: compass calibrated this session (CAL took ~70s of tilt-game). Compass now produces real headings (was guarded as `COMPASS,-1,0` pre-cal). Accel reads y≈1g confirming vertical mount. Pose composite + gesture working.

**Environment**: temperature ✓, buttons A/B ✓, V2 logo touch ✓.
...
**New file:** `firmware/sense_debug.py` — standalone µbit diagnostic for V2 sensors. Bar-graph display of light + sound levels with event-API latch. Bypass-radio sensor verification: flash to any single µbit, no Cutebot or bridge needed. Versioned via boot-scroll signature (current: v4). Built-in feature-detect for `microphone.was_event` so it doesn't crash on older MicroPython builds.
