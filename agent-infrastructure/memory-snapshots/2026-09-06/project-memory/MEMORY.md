...
- [AI PD Tracker](project_ai-pd-tracker.md) — ~/Projects/ai-pd-tracker/ FastAPI+SQLite curriculum tracker + Field Guide workbench; remote restructured 26.8.4 (Track P, Phase 3, 41 lessons, 106 tests); audit triage 26.9.5 → plan there-are-two-documents-hazy-peach.md; Wave 0 merged (PR #5); Wave 1 coach assessor PR #6 READY (4-way 13/13; must_hold 7/13) MERGED; Waves 2a/2b + currency bootstrap + link hygiene ALL MERGED (PRs #7,#10,#9,#11; #8 auto-closed by stacked-base deletion) — 45 lessons, 187h+4h gauntlet, model-watch populated (AA figures corrected in review); gold set 19 cases (6 adversarial by-construction; assessor 19/19 ×3, 0 false-PASS); Wave 3 review-retrieval gate MERGED (PR #14, 143 tests); P2/P3 applied + Day 5 temperature/prefill correction (PR #13); gauntlet spec + Track P lesson (PR #15; 46 lessons, 191h); ALL pre-Day-1 waves done; login LaunchAgent com.aipd.tracker installed + Desktop launcher (PR #16); final cleanup PR #17 (real assertion, README trust model); Opus 5 switch PR #18 26.9.6 — found `.env` override had kept Sonnet 4.6 running through every calibration run; Opus gold set 18/19 ×3, 0 false-PASS, case 04 false-hold; next = just-in-time only: slot-1 case day-before Sprint gate, slots 2–5, gauntlet artifacts; schedule weekly currency run; evidence now comes from real learner use — full detail in file
...
- [Verify resolved config, not source](feedback_verify-resolved-config-not-source.md) — a default-change check must read the runtime-resolved value or the field recorded in outputs; `.env` silently overrode the code default and every calibration run ran on the old model (26.9.6)
- [No tautological assertions](feedback_no-tautological-assertions.md) — never `or True` / permanently-green asserts; compute the expected value from the code's own rule or assert a weaker real property (26.9.5)
