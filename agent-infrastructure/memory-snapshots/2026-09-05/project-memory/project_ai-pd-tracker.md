...
**Next.** Merge PR #5 → Wave 1 (assessor + `must_hold` gold set; separate scoped session, sigma-build candidate; due before the Day 4 coach check) → one-off currency run → Wave 2 before P.1. Wave 2 also carries the E2 "read everything it produced" reword in P.4 and the coach's "can read the code that comes back" line.
