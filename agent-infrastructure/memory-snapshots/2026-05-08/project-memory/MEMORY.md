...
- [Cutebot MCP](project_cutebot-mcp.md) — ~/Projects/cutebot-mcp/ MCP server. v0.2.1 LIVE 26.5.8: 15/22 actions verified end-to-end (incl scan_distance polar sweep, all 11 senses, headlight color cycle, beep, drive pivot). Live-discovered firmware bugs fixed: handle() exceeded MP per-function bytecode → split into 7 sub-dispatchers; compass.heading() auto-triggered hang → guarded; get_strength() missing on this MP build → manual sqrt. Currently 2 µbits (bridge+robot); beacon role empty (needs 3rd µbit for hide-and-seek). 4 commits, working tree clean.
