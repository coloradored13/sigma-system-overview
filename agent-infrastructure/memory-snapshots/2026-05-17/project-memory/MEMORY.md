...
- [Cutebot MCP](project_cutebot-mcp.md) — ~/Projects/cutebot-mcp/ MCP server. v0.2.1 LIVE 26.5.8: 15/22 actions verified end-to-end. Live-discovered firmware bugs fixed: handle() bytecode split, compass auto-cal guard, get_strength() manual sqrt. **🔧 LEFT MOTOR DEAD (confirmed across battery states; bot can pivot but not translate).** Physical check needed: lift wheel, inspect JST motor connector. 2 µbits in play; beacon role empty. 4 commits, working tree clean.
