...
**🔧 Hardware fault discovered 2026-05-08 (after live exploration):**
Left motor / left drivetrain is non-functional. Confirmed by isolation testing across multiple battery states:
- Drive(50, 0, ...) — left motor only — produces no visible motion (even with fresh AAAs).
- Drive(0, 50, ...) — right motor only — produces clean ~90° CCW pivot.
- Drive(50, 50, ...) — both — degenerates into pure right-motor behavior; bot spins CCW instead of translating forward. This was the source of the "stayed in one spot pivoting on one tire" behavior during the explore loop.
- Drive(-50, -50, ...) — reverse — bot pivots CW (right motor reversing alone). Symmetric to forward case.
Battery brownout was a confounder — old batteries also caused dropouts under load, masking the deeper issue. Fresh AAAs ruled it out: left wheel still doesn't engage.
Likely causes (in order of cost): jammed left gearbox / debris at axle / loose JST motor connector at the Cutebot board / dead motor coil / failed left channel on the I2C motor driver IC. **Recommended physical checks before next session:**
1. Lift bot, spin left wheel by hand vs. right wheel — feel for binding (mechanical) vs. free spin (electrical).
2. Inspect the 2-pin JST connector at the left motor — reseat if loose.
3. If left wheel spins free in air during D,40,0,800 → motor + wiring fine, friction issue. If still no spin in air → electrical/motor.
**Software workaround until repaired:** the bot can still pivot in place (right-motor-only), so scan_distance, RSSI scans, and pivot-based exploration patterns work. It cannot translate across the floor.
