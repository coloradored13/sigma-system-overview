...
- [Personas Audit](project_persona-audit.md) — SRS Acquiom loan-agency persona package v1→v1.2 owner-built 26.5.12, 13 personas, evidence-basis tagging, /Users/bjgilbert/Documents/personas-audit/, secondary-research closure of v1.1 gaps via sigma-retrieve + sigma-single
- [K-shape Ultra Report](project_k-shape-ultra-report.md) — synthesized 13,995-word K-shape opportunity scan 26.5.17, /Users/bjgilbert/Documents/k-shape-ultra-report-2026-05-17.{md,docx}, combines best moves from 5 inputs (sigma-review 24 + Claude DR 22 + sigma-single 20 + ChatGPT DR 16 + Gemini DR 11) against sigma-review's framework backbone
...
- [AI PD Tracker](project_ai-pd-tracker.md) — ~/Projects/ai-pd-tracker/ FastAPI+SQLite curriculum tracker, 10 steps shipped 26.4.20 + verified source ledger (23/23) + curriculum schema (track/gate/notebook_id) + 6 NotebookLM manifests + live NotebookLM build (notebook id d482a634, Stream 2 closed) + DEFERRED.md 26.4.29-30 + Track B 8/8 Operator Foundations shipped 26.5.7 commit 08a5426 (41.5h, capstone=Field Guide self-application = velvety-swinging-coral Stream 3 closed) + Field Guide v2 citation-inserted (10 insertions, all PASS-verified) shipped 26.5.7 commit d458f22 (voice-polish pending = Stream 4). Private repo coloradored13/ai-pd-tracker, coach validated (shares sigma-verify API key)
- [Cutebot MCP](project_cutebot-mcp.md) — ~/Projects/cutebot-mcp/ MCP server. v0.2.3 LIVE on hardware 26.5.23 (uncommitted, phase-gate blocked by parallel sigma session): full 22-action systems check + radio investigation. **Radio "wedge" DEBUNKED via 1792-cmd 5-min sweep** — was ~5% baseline 2.4GHz drop rate × double-attempt --ping-robot. Fix: bridge.py default retries 1→3. **BLE migration NOT needed** — current architecture sound. Motors fine (hair on axle was friction culprit, not "dead left motor"). Watchdog patched (long drives work). Light/sound NOT broken — env-occluded by mount. Headlight wire protocol uses string side "L"/"R"/"B" (memory was wrong about numeric). Compass calibrated. Diagnostics (DIAG/RST/auto-recovery/heartbeat) shipped as safety net. 6 committed + 2 dirty.
...
- !source-validation 26.4.29: installed + integrated as sigma-single Stage 3.5 Source Bias Probe (capture/conflict/framing-bias ⊥ sigma-retrieve's authority/recency); sigma-retrieve description rewritten to front standalone deep-research framing (commit 08f8241). Does NOT percolate to sigma-review — that's a separate integration if needed.
- [Sigma-single triangulation gap](project_sigma-single-triangulation-gap.md) — sigma-single's live market-size figures lean on aggregator-tier sources (Mordor, FMI, Sacra) without down-weighting in ranking math. Fix is sigma-review's Section 12 triangulation policy: ≥2 source categories for load-bearing magnitudes; single-source figures flagged as directional, excluded from ranking (26.5.17 benchmark, see [[reference_sigma-review-benchmark-2026-05-17]])
...
- [TaskCreate before TeamCreate leak](feedback_taskcreate-before-teamcreate-leak.md) — pre-team TaskCreate strands tasks in session-scope and bleeds into agent inboxes as misrouted lead-track work; call TeamCreate first or skip task tracking (26.5.8)
- [Avoid profile anchoring](feedback_avoid-profile-anchoring.md) — when user asks for evidence-based landscape/opportunity analysis, do NOT narrow scope to user's profile/skills/domain; deliver evidence neutral. Personalizing feels helpful but is a narrowing bias. Default opportunity types: tech/SaaS, services-for-affluent, blue-collar shovel-labor, skilled trades, SMB acquisition, trade-down enablers, healthcare home services. (26.5.17)
- [Sigma-review restart hygiene](feedback_sigma-review-restart-hygiene.md) — on /sigma-review with active unfinished WS, FULLY wipe state (workspace + chain-status + revert un-validated promotions) BEFORE investigating prior framing; do NOT surface prior hypotheses to user as resume options — anchoring contamination. End current session, instruct user to start fresh. (26.5.21)
- [XVERIFY is cross-check not proof](feedback_xverify-is-cross-check-not-proof.md) — LLM-checking-LLM is ouroboros; primary verification = web/source research, XVERIFY = supplemental sanity check only. T1 tier requires primary external source, not just XVERIFY-agreement. Update DA agent-def + spawn templates + §2h. (26.5.27)
